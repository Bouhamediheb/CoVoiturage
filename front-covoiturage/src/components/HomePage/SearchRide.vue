          ...
          <div class="ride-picker">
            <div class="input-box">
              ...
              <div v-if="startPointFiltered.length > 0" class="autocomplete">
                <ul>
                  <li v-for="(city, index) in startPointFiltered" :key="index" @click="selectStartPoint(city)">{{ city }}</li>
                </ul>
              </div>
            </div>
            <br>
            <div class="input-box">
              <i class="fa fa-map-marker"></i>
              <input type="text" placeholder="Ending Point" v-model="endPoint" @input="filterEndPoint" class="ride-picker__input">
              <div v-if="endPointFiltered.length > 0" class="autocomplete">
                <ul>
                  <li v-for="(city, index) in endPointFiltered" :key="index" @click="selectEndPoint(city)">{{ city }}</li>
                </ul>
              </div>
            </div>
